...
ifneq ($(wildcard $(strip $(SOURCE_DIR)/tools/gcc/linux/gcc-arm-none-eabi/lib64 )),)
	export LD_LIBRARY_PATH := $(SOURCE_DIR)/tools/gcc/linux/gcc-arm-none-eabi/lib64:$(LD_LIBRARY_PATH)
endif
...
$(BUILD_DIR)/%.o: $(SOURCE_DIR)/%.c
	...
	@if [ -n "$(OVERRIDE_CFLAGS)" ]; then \
		...
		$(CC) $(OVERRIDE_CFLAGS) -c $< -o $@ 2>>$(ERR_LOG); \
                ...
                echo $(CC) $(CFLAGS) -MMD -MF $(basename $@).d $@ >> $(BUILD_LOG); \
                $(CC) $(CFLAGS) -MMD -MF $(basename $@).d  -c $< -o $@ 2>>$(ERR_LOG); \
	fi; \
	if [ "$$?" != "0" ]; then \
...
$(BUILD_DIR)/%.o: $(SOURCE_DIR)/%.s
	...
	@if [ -e "$@" ]; then rm -f "$@"; fi
	@if [ -n "$(OVERRIDE_CFLAGS)" ]; then \
		$(CC) $(OVERRIDE_CFLAGS) -c  -x assembler-with-cpp $< -o $@; \
	else \
