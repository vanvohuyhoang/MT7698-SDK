...
static void _uart_event(hal_uart_callback_event_t event, void *user_data)
{
    ...
    switch (event) {
        case HAL_UART_EVENT_WAKEUP_SLEEP: //add by gopal.yang for align code to new implement
#ifdef HAL_SLEEP_MANAGER_ENABLED
#if configUSE_TICKLESS_IDLE == 2
            hal_sleep_manager_lock_sleep(uart_rx_eint_sleep_handle);
#endif
...
extern bool log_write_to_dump_buffer(char *buf, int32_t len);
#endif
...
int log_write(char *buf, int len)
{
    ...
    if (_uart_dma_blocking_is_safe()) {
        ...
        } while (left && xSemaphoreTake(_g_semaphore_tx, 50));
