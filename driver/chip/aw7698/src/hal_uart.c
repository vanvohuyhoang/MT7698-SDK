...
static const uint32_t g_uart_baudrate_map[HAL_UART_BAUDRATE_MAX] = {110, 300, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600, 2000000, 3000000,3200000};
...
void uart_restore_all_registers(void)
{
    ...
    for (uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
        ...
        if (g_uart_irq_pending_status[uart_port] != 0) {
