...
#if defined(SF_DAL_ZBIT)
        case SF_DAL_TYPE_ZBIT:
            result  = SF_DAL_Init_WINBOND(driver, D);
            result |= SF_DAL_Init_WINBOND_OTP(driver, D);
            break;
#endif
...
ATTR_TEXT_IN_RAM int32_t SF_DAL_ProgramDataWrapper(void *MTDData, void *Address, void *Data, uint32_t Length)
{
...
ATTR_TEXT_IN_RAM int32_t SF_DAL_EraseBlockWrapper(void *MTDData, uint32_t BlockIndex)
{
    return FS_UNSUPPORTED_DRIVER_FUNCTION;
}
...
ATTR_TEXT_IN_RAM int32_t SF_DAL_LockEraseBlkAddr(void *MTDData, void *BlkAddr, uint32_t ACTION)
{
    SF_MTD_Data        *D;
