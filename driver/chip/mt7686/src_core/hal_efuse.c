...
#define HAL_EFUSE_SW_DEF_START_ADDR         (0x800)
#define HAL_EFUSE_SW_DEF_END_ADDR           (0x900)
#define HAL_EFUSE_SW_DEF_SINGLE_RW_CON      (0xA20A0028)
#define HAL_EFUSE_SW_DEF_SINGLE_WD_DATA     (0xA20A0020)
#define HAL_EFUSE_SW_DEF_SINGLE_RD_DATA     (0xA20A0024)
...
hal_efuse_status_t hal_efuse_write(uint32_t magic, uint16_t addr, const uint8_t *buf, uint8_t len)
{
    ...
    if((addr >= HAL_EFUSE_SW_DEF_START_ADDR) && (addr < HAL_EFUSE_SW_DEF_END_ADDR)) {
        is_sw_def = true;
        real_addr = addr - HAL_EFUSE_SW_DEF_START_ADDR;
    } else {
    ...
    if (!is_sw_def) {
        ...
        for(i = 0; i < len; i += 4) {
            DRV_WriteReg32(HAL_EFUSE_SW_DEF_SINGLE_RW_CON, ((real_addr + i) >> 2) + 64);
            memcpy(&value, buf + i, sizeof(uint32_t));
            DRV_WriteReg32(HAL_EFUSE_SW_DEF_SINGLE_WD_DATA, value);
...
hal_efuse_status_t hal_efuse_read(uint16_t addr, uint8_t *buf, uint8_t len)
{
    ...
    if ( !is_sw_def) {
        ...
        for (i = 0; i < len; i += 4) {
            /* Read EFUSE data */
            DRV_WriteReg32(HAL_EFUSE_SW_DEF_SINGLE_RW_CON, (((real_addr + i) >> 2) + 64) | 0x100);
            while(EFUSE_REGISTER->EFUSEC_CON1.b.VLD != 1);
            value = DRV_Reg32(HAL_EFUSE_SW_DEF_SINGLE_RD_DATA);
            memcpy(buf + i, &value, sizeof(uint32_t));
        }
    }
