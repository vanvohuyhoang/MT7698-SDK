...
{
    uint8_t index = 0, i;
    uint32_t mask, name_len, name_Illegal = 0;
...
bool sleep_management_check_handle_status(uint8_t handle_index)
{
    ...
    if (((sleep_management_handle.user_handle_resoure >> (handle_index - SLEEP_LOCK_USER_START_ID)) & 0x01) == 1) {
        return true;
    } else {
        return false;
    }
...
uint32_t sleep_management_get_lock_sleep_handle_list(void)
{
    ...
    printf("\r\n[Sleep Management]lock sleep handle list : 0x%x\r\n", (unsigned int)lock_sleep_request);
