...
ATTR_TEXT_IN_TCM static int flash_Switch_Device_Mode(uint32_t mode)
{
    ...
    if (support_flash_id == SF_TYPE_WINBOND || support_flash_id == SF_TYPE_GD) {
        if (flash_read_sr2(&qbit) < 0) {
            assert(0);
        }
        //SR2.1 is Q bit
        if (((qbit & 0x2) == 0x2 && mode == SPI) ||
            ((qbit & 0x2) == 0x0 && ((mode == SPIQ) || (mode == QPI)))) {
            modechange = 1;
        } else {
            modechange = 0;
        }
    } else if (support_flash_id == SF_TYPE_MXIC || support_flash_id == SF_TYPE_MICRON) {
        if (flash_read_sr(&qbit) < 0) {
            assert(0);
        }
        //SR1.6 is Q bit
        if (((qbit & 0x40) == 0x40 && mode == SPI) ||
            ((qbit & 0x40) == 0x0 && ((mode == SPIQ) || (mode == QPI)))) {
            modechange = 1;
        } else {
            modechange = 0;
        }
...
ATTR_TEXT_IN_TCM int flash_write(const unsigned char *buf, unsigned int to, int len)
{
    ...
    while (len > 0) {
        ...
        page_offset = 0;
        p_data_first = Buf;
        data = *(volatile uint8_t *)((kal_uint32)Buf + page_size - 1);
...
retry:
        ...
        do {
            result = SF_DAL_CheckReadyAndResume(0,CM4_EXEC_IN_PLACE_BASE|((kal_uint32)To+page_size-1), data);
        } while (0 == result);

        if (result != 1) {
        	   /* It is not necessary to do flash_write_disable as WEL is automatically reset after program finished */
            return -6;  // program failed
        }
