...
/* Private macro ------------------------------------------------------------*/
#define EXCEPTION_UPGRADE_DONE                      0xdeaddead

/* Private variables --------------------------------------------------------*/
/* exception handler's stack */
static uint32_t exception_upgrade_flag_dsp = 0;
...
{
    if((hal_core_status_read(HAL_CORE_DSP0) > HAL_CORE_OFF) && (exception_upgrade_flag_dsp != EXCEPTION_UPGRADE_DONE))
    {
#ifdef HAL_WDT_MODULE_ENABLED
        /* Tigger NMI exception to upgrade exception */
...
#endif
        exception_upgrade_flag_dsp = EXCEPTION_UPGRADE_DONE;
    }
}
...
void exception_forceddump_dsp1(void)
{
    if((hal_core_status_read(HAL_CORE_DSP1) > HAL_CORE_OFF) && (exception_upgrade_flag_dsp != EXCEPTION_UPGRADE_DONE))
    {
#ifdef HAL_WDT_MODULE_ENABLED
        /* Tigger NMI exception to upgrade exception */
        hal_wdt_config_t wdt_config;
