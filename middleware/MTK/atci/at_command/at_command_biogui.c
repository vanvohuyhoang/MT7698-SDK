...
#define LOGMSGIDE(fmt,cnt,arg...)   LOG_MSGID_E(atci_biogui ,"[biogui]"fmt,cnt,##arg)
#define LOGMSGIDW(fmt,cnt,arg...)   LOG_MSGID_W(atci_biogui ,"[biogui]"fmt,cnt,##arg)
#define LOGMSGIDI(fmt,cnt,arg...)   LOG_MSGID_I(atci_biogui ,"[biogui]"fmt,cnt,##arg)
                ...
                if (i % 2 == 0) {
                    ...
                    if (data_count != 0 && (data_count % (PPG_SAMPLE_RATE * PPG_SAMPLE_RUN_SEC)) == 0 ) {
                        ...
                        if (led_count <= 0xf0) {
                            ...
                            LOGMSGIDI("data_count %ld, ambdac level %lu \r\n",2, data_count, ambdac_count);
                            led_count = 0;
                            ambdac_count++;
                            vsm_driver_set_led_current(VSM_LED_1,led_count);
                            vsm_driver_set_led_current(VSM_LED_2,led_count);
...
int32_t signal_noise_compute(vsm_signal_t signal, double *signal_noise, bool is_odd, bool is_amplitude)
{
    ...
    if(signal == VSM_SIGNAL_EKG || signal == VSM_SIGNAL_PPG1) {
        ...
        LOGMSGIDI("prepare init \r\n", 0);

#ifndef GOLDEN_TEST
        /* initialize vsm */
        vsm_driver_init();
        LOGMSGIDI("prepare set signal %d \r\n",1, signal);
...
{
    LOGMSGIDI("atci_biogui_check_task_life type %d \r\n",1, type);

    switch(type){
        case TYPE_PPG1:
            LOGMSGIDI("atci_biogui_check_task_life PPG1 %d \r\n",1, PPG1);
            if(PPG1) {
                return true;
            } else {
                return false;
        ...
        case TYPE_PPG2:
            LOGMSGIDI("atci_biogui_check_task_life PPG2 %d \r\n",1, PPG2);
            if(PPG2) {
                return true;
            } else {
                return false;
        ...
        case TYPE_EKG:
            LOGMSGIDI("atci_biogui_check_task_life EKG %d \r\n",1, EKG);
            if(EKG) {
                return true;
            } else {
                return false;
        ...
        case TYPE_BISI:
            LOGMSGIDI("atci_biogui_check_task_life BISI %d \r\n",1, BISI);
            if(BISI) {
                return true;
            } else {
                return false;
...
void atci_biogui_task_main(void *pvParameters)
{
    ...
    while(1) {
        ...
        zero_cnt = 0;
        LOGMSGIDI("collecting task start ... signal_type %d, sample %d\r\n",2, signal_type, sample);
        ...
        while(1)
        {
            ...
            LOGMSGIDI("collecting task pass block condition ... \r\n", 0);
            ...
            LOGMSGIDI("1.output_len %d, sample %d \r\n",2, output_len, sample);
            if (output_len > 1) {            
                output_len = (output_len >= sample)?sample:output_len;
                LOGMSGIDI("2.output_len %d, sample %d \r\n",2, output_len, sample);
                ...
                LOGMSGIDI("output_len %ld, sample %ld \r\n",2, output_len, sample);
                //vTaskDelay(10);
            } else {
                zero_cnt ++;
                if(zero_cnt > 100) {
...
atci_status_t atci_cmd_hdlr_biogui(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    LOGMSGIDI("atci_cmd_hdlr_biogui \r\n", 0);
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+EKEYURC=<op>  the handler need to parse the parameters
            ...
            if (*type == '0') {                
                ...
                if (s1) {
                    ...
                    LOGMSGIDI("data.addr 0x%x, data.reg 0x%x\r\n",2, data.addr, data.reg);
                ...
                if(s1 && s2) {
                    ...
                    data.data_buf = (uint8_t *)&reg_value;
                    data.length = sizeof(reg_value);
                    LOGMSGIDI("data.addr 0x%x, data.reg 0x%x, reg_value 0x%x\r\n",3, data.addr, data.reg, reg_value);
                    ...
                    LOGMSGIDI("no addr value or register value... \r\n", 0);
                    strcpy((char *)response.response_buf, "ERROR\r\n");
                    response.response_len = strlen((char *)response.response_buf);
                    atci_send_response(&response);
                }
                ...
                if(s1 && s2) {
                    ...
                    if ( (addr2 > addr1) && (addr2 % 4 == 0) && (addr1 % 4 == 0)) {
                        ...
                        LOGMSGIDI("addr1 0x%x, addr2 0x%x, temp_addr2 0x%x, non_exist_item %d\r\n",4,
                        ...
                        if (temp_addr2 != 0) {
                            ...
                            addr2 = temp_addr2;
                            //count = count2;
                            LOGMSGIDI("slave 0x33 addr1 0x%x, addr2 0x%x\r\n",2, addr1, addr2);
                            memset(&response, 0, sizeof(response));
                            for (i = 0; i < count2; i ++) {
                                if (addr1 == 0x33B8 || addr1 == 0x33BC) {
                                    addr1 +=4;
                ...
                if(s1 && s2) {                    
                    ...
                    LOGMSGIDI("start collect type %d ,sample %d\r\n",2, signal_type, samples);
                    //enable signal, type: 0:ppg1 1:ppg2 2:ekg
                    LOGMSGIDI("PPG1 %d, PPG2 %d, EKG %d, BISI %d\r\n",4, PPG1, PPG2, EKG, BISI);
                    switch(signal_type){
                    ...
                    if (enable_get_sram) {
                        ...
                        LOGMSGIDI("task_para.type %d ,task_para.sample %d\r\n",2, task_para.type, task_para.sample);
                    ...
                    LOGMSGIDI("no type or sample value... \r\n", 0);
                    strcpy((char *)response.response_buf, "ERROR\r\n");
                    response.response_len = strlen((char *)response.response_buf);
                }
#else
            ...
            /* This is original smt test case*/
                ...
                if(s1) {
                    ...
                    if (test_item == 1) {                        
                        ...
                        hal_gpio_set_output(GPIO_MT2511_PPG_VDRV_EN, HAL_GPIO_DATA_LOW);
                        LOGMSGIDI("BIO read sram successfully... \r\n", 0);
                        response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
        				response.response_len   = 0;
                    } else if (test_item == 2) {                        
                        /* turn off */
                        vsm_driver_deinit();
                        LOGMSGIDI("disable MT2511 successfully \r\n", 0);
                        response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
                        response.response_len   = 0;
                    } 
                    #ifdef MT2511_SMT_MODE
