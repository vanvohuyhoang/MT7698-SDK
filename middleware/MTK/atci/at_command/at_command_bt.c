...
atci_status_t atci_cmd_hdlr_bt_relay(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_EXECUTION: { /*AT+EBTER=<port>*/
            ...
            LOG_MSGID_I(atci_bt, "port:%d",1, port);
            ...
            result = false;
            /*check port is atci port or not*/
...
#ifdef MTK_PORT_SERVICE_ENABLE
            LOG_MSGID_I(atci_bt, "en relay with port srv, port: %d",1, port);
            result = bt_enable_relay_mode_with_port_service((serial_port_dev_t)port);
#endif

            LOG_MSGID_I(atci_bt, "enter relay mode result: 0x%x",1, result);
            if (g_atci_uart_port == (hal_uart_port_t)port) {
                serial_port_setting_uart_t uart_setting;
                
                uart_setting.baudrate = HAL_UART_BAUDRATE_921600;
                serial_port_config_write_dev_setting((serial_port_dev_t)port, (serial_port_dev_setting_t *)&uart_setting);
                return ATCI_STATUS_OK;
            }
            if (result) {
                output.response_len = 0;
