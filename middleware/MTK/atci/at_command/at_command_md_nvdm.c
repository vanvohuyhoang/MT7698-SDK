...
static void dump_data_item_info(nvdm_modem_data_item_info_t *info_list, uint32_t count)
{
    ...
    for (i = 0; i < count; i++) {
        LOG_MSGID_I(common, "Dump data item:%d(%d)",2, i, count);
        LOG_MSGID_I(common, " [AREA]=0x%x",1, info_list[i].area);
...
atci_status_t atci_cmd_hdlr_nvdm_modem(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    LOG_MSGID_I(common, "atci_cmd_hdlr_nvdm_modem\n", 0);
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+ENVDMMD=<op>  the handler need to parse the parameters
            ...
            if (strstr((char *)parse_cmd->string_ptr, "AT+ENVDMMD=0") != NULL) {
                ...
                LOG_MSGID_I(common, "AREA = %d",1, area);
                ...
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_read_data_item status = %d",1, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                LOG_MSGID_I(common, "nvdm_read_data_item length = %d",1, length);
                {
                    ...
                    for (i = 0; i < length; i++) {
                        LOG_MSGID_I(common, "0x%x",1, resonse.response_buf[i]);
                    }
                }
                ...
                LOG_MSGID_I(common, "atci_cmd_hdlr_nvdm_modem: write\n", 0);
                parameter = strtok_r(parse_cmd->string_ptr, delim, &saveptr);
                ...
                LOG_MSGID_I(common, "attribute = %d",1, attr);
                ...
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_modem_write_data_item status = %d",1, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                ...
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_modem_delete_data_item status = %d",1, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                ...
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_modem_reset_data_item status = %d",1, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                ...
                LOG_MSGID_I(common, "[QUERY][AREA:%d] data_number = %d",2, area, data_number);
                ...
                if (status == NVDM_MODEM_STATUS_OK) {
                    ...
                    LOG_MSGID_I(common, "[ERROR][QUERY][AREA=%d]query data item error:%d",2, area, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                }
                vPortFree(p_info);
                ...
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_modem_query_data_item_area status = %d",1, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                ...
                LOG_MSGID_I(common, "dump_data_cont=0x%x, ...",1, buffer[0]);

                status = nvdm_modem_write_mini_dump_data(minidump_buf, length);
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_modem_write_mini_dump_data status = %d",1, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    vPortFree(minidump_buf);
                    break;
                } else {
                    nvdm_modem_query_mini_dump_number((uint8_t *)&dump_num);
                    LOG_MSGID_I(common, "write success:len=%d,dump_num=%d",2, length, dump_num);
                ...
                LOG_MSGID_I(common, "Total dump_num=%d",1, dump_num);
                if (dump_num == 0) {
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                    break;
                }
                ...
                for (dump_idx = 0; dump_idx < dump_num; dump_idx++) {
                    ...
                    do {
                        ...
                        if (status != NVDM_MODEM_STATUS_OK || mini_dump_data_verify(buffer, buf_size) < 0) {
                            LOG_MSGID_I(common, "[ERROR] read/verify mini-dump error idx=%d",1, dump_idx);
                ...
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_modem_read_mini_dump_data status = %d",1, status);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                }
                ...
                LOG_MSGID_I(common, "dump_idx = %d",1, dump_idx);

                status = nvdm_modem_clean_mini_dump_data(dump_idx);
                if (status != NVDM_MODEM_STATUS_OK) {
                    LOG_MSGID_I(common, "[ERROR]nvdm_modem_clean_mini_dump status=%d, idx=%d",2, status, dump_idx);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
                    break;
                } else {
                    nvdm_modem_query_mini_dump_number(&dump_num);
                    LOG_MSGID_I(common, "clean success:idx=%d,dump_num=%d",2, dump_idx, dump_num);
                }
                resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                #else
                resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
