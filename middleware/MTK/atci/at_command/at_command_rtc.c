...
atci_status_t atci_cmd_hdlr_rtc(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    LOG_MSGID_I(RTC_ATCI, "atci_cmd_hdlr_rtc \r\n", 0);
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+ERTCREG=<op>  the handler need to parse the parameters
            ...
            if (strncmp(parse_cmd->string_ptr, "AT+ERTCREG=0,", strlen("AT+ERTCREG=0,")) == 0) {
                ...
                end_pos = NULL;
                LOG_MSGID_I(RTC_ATCI, "read register address:0x%x\r\n",1, input_addr);

                /* read data of input register address */
                //read_value = rtc_get_register(input_addr); //mark by lei bypass build error
                ...
                mid_pos = NULL;
                end_pos = NULL;
                LOG_MSGID_I(RTC_ATCI, "register address:0x%x, set register value:0x%x\r\n",2, input_addr, input_value);
                /* write input data to input register address*/
                //rtc_set_register(input_addr, input_value);  //mark by lei bypass build error
                snprintf((char *)response.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, "+ERTCREG:addr0x%x,value0x%x\r\n", input_addr, input_value);
                /* ATCI will help append "OK" at the end of response buffer  */
                response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
            }
#ifdef MTK_BUILD_SMT_LOAD
...
static bool rtc_alarm_test(int second)
{
    ...
    if (false == set_alarm_time(time.rtc_year, time.rtc_mon, time.rtc_day, time.rtc_hour, time.rtc_min, time.rtc_sec, time.rtc_week)) {
        LOG_MSGID_E(RTC_ATCI, "set_alarm_time fail", 0);
        return false;
    }
    if (false == set_alarm_handle_cb()) {
        LOG_MSGID_E(RTC_ATCI, "set_alarm_handle_cb fail", 0);
        return false;
    }
    ...
    if (false == rtc_alarm_happened)
        LOG_MSGID_E(RTC_ATCI, "rtc_alarm_test timeout", 0);
...
static void rtc_set_rtc_mode(void)
{
    ...
    *(volatile uint16_t *)0xA21e0078 = value;
    ...
    value = *(volatile uint16_t *)0xA21e0078;
    if ((value & 0x100) == 0) {
        LOG_MSGID_E(RTC_ATCI, "rtc_mode set fail\r\n", 0);
    } else {
        LOG_MSGID_E(RTC_ATCI, "rtc_mode set ok\r\n", 0);
    }
}
...
static void rtc_clear_rtc_mode(void)
{
    ...
    *(volatile uint16_t *)0xA21e0078 = value;
    ...
    value = *(volatile uint16_t *)0xA21e0078;
    if ((value & 0x100) != 0) {
        LOG_MSGID_E(RTC_ATCI, "rtc_mode clear fail\r\n", 0);
    } else {
        LOG_MSGID_E(RTC_ATCI, "rtc_mode clear ok\r\n", 0);
    }
}
#endif
#endif
