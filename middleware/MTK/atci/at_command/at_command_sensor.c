...
#define LOGMSGIDE(fmt,cnt,arg...)   LOG_MSGID_E(atcmd, "SENSOR_ATCMD: "fmt,cnt,##arg)
#define LOGMSGIDW(fmt,cnt,arg...)   LOG_MSGID_W(atcmd, "SENSOR_ATCMD: "fmt,cnt,##arg)
#define LOGMSGIDI(fmt,cnt,arg...)   LOG_MSGID_I(atcmd ,"SENSOR_ATCMD: "fmt,cnt,##arg)
...
double custom_atan(double x){
    LOGMSGIDI("atan(%ld/10000) = %ld/10000\r\n",2, (int32_t)(x*10000),(int32_t)(sensor_atan(x)*10000) );
...
int32_t sensor_send_digest_callback(sensor_data_t *const output)
{
    if ((total_data_count++) % 100 == 0) {
        LOGMSGIDI("sensor type = %lu , value = ( %ld , %ld , %ld ) (%ld) , timestamp = %lu \r\n",6,
    ...
    switch(output->data[0].sensor_type){
        case SENSOR_TYPE_ACCELEROMETER:
            ...
            if(g_hide_acc == false){
                if ((acc_count++) % 100 == 0) {
                    LOGMSGIDI("[QA_LOG] accelerometer data = (%ld,%ld,%ld) mm/s^2 , accuracy = %ld , timestamp = %lu \r\n",5,
        ...
        case SENSOR_TYPE_GYROSCOPE:
            if ((gyro_count++) % 100 == 0) {
                LOGMSGIDI("[QA_LOG] gyroscope data = (%ld,%ld,%ld) m-degree/s , accuracy = %ld , timestamp = %lu \r\n",5,
        ...
        case SENSOR_TYPE_MAGNETIC_FIELD:
            if ((mag_count++) % 100 == 0) {
                LOGMSGIDI("[QA_LOG] magnetic field degree = %ld degree , accuracy = %ld , timestamp = %lu \r\n",3,
        ...
        case SENSOR_TYPE_PRESSURE:
            if ((baro_count++) % 100 == 0) {
                LOGMSGIDI("[QA_LOG] pressure data = %ld Pa  , timestamp = %lu \r\n",2,
        ...
        case SENSOR_TYPE_PROXIMITY:
            ...
            if(output->data[0].distance == 0){
                LOGMSGIDI("[QA_LOG] proximity data = Near  , timestamp = %lu \r\n",1,output->data[0].time_stamp);
            }else{
                LOGMSGIDI("[QA_LOG] proximity data = Far  , timestamp = %lu \r\n",1,output->data[0].time_stamp);
            }
            prox_count++;
            break;
        default :
            LOGMSGIDI("SQC not inlcude this sensor type\r\n", 0);
            break;
    }
...
bool baro_auto_test(int32_t base_pressure){
    ...
    LOGMSGIDI("[QA_LOG] baro up bound = %ld , low bound = %ld \r\n",2,baro_up_bound,baro_low_bound);
    ...
    LOGMSGIDI("[QA_LOG] pressure total data = %ld Pa  \r\n",1,baro_data);
    baro_data = baro_data/BARO_AUTO_TIMES;
    LOGMSGIDI("[QA_LOG] pressure avg data = %ld Pa  \r\n",1,baro_data);
    if( (baro_data>baro_up_bound) || (baro_data<baro_low_bound) ){
        LOGMSGIDI("[QA_LOG] auto test fail baro value out of range\r\n", 0);
        return false;
    }
    return true;
}
...
bool gyro_auto_test(){
    ...
    LOGMSGIDI("[QA_LOG] gyroscope total = (%ld,%ld,%ld) m-degree/s \r\n",3,x,y,z);
    x = x/GYRO_AUTO_TIMES;
    y = y/GYRO_AUTO_TIMES;
    z = z/GYRO_AUTO_TIMES;
    LOGMSGIDI("[QA_LOG] gyroscope avg   = (%ld,%ld,%ld) m-degree/s \r\n",3,x,y,z);

    if( (x>GYRO_SPEC_UP_BOUND) || (x<GYRO_SPEC_LOW_BOUND) ){
        LOGMSGIDI("[QA_LOG] auto test fail x value out of range\r\n", 0);
        return false;
    }

    if( (y>GYRO_SPEC_UP_BOUND) || (y<GYRO_SPEC_LOW_BOUND) ){
        LOGMSGIDI("[QA_LOG] auto test fail y value out of range\r\n", 0);
        return false;
    }
...
int sanity_check(int sensor_type)
{
    ...
    if(sensor_type == SENSOR_TYPE_PROXIMITY){
        ...
        for(i=0;i<DATA_CHECK_TIMES;i++){
            ...
            LOGMSGIDI("Sensor(%d) DATA %d. (%ld,%ld,%ld) \r\n",5,sensor_type,i,output.data[0].value[0],output.data[0].value[1],output.data[0].value[2]);
            if(output.data[0].value[0]!=0 || output.data[0].value[1]!=0 || output.data[0].value[2]!=0 ){
                LOGMSGIDI("Sensor(%d) DATA not zero , pass\r\n",1,sensor_type);
                is_pass = 1;
                break;
            }
        }
    ...
    LOGMSGIDI("Sensor(%d) OFF\r\n",1,sensor_type);
    if(is_pass == 0){
        switch(sensor_type){
        case SENSOR_TYPE_ACCELEROMETER:
            LOGMSGIDI("[QA_LOG] Accelerometer Sensor FAIL\r\n", 0);
            break;
        case SENSOR_TYPE_GYROSCOPE:
            LOGMSGIDI("[QA_LOG] Gyroscope Sensor FAIL\r\n", 0);
            break;
        case SENSOR_TYPE_MAGNETIC_FIELD:
            LOGMSGIDI("[QA_LOG] Magnetic Field Sensor FAIL\r\n", 0);
            break;
        case SENSOR_TYPE_PRESSURE:
            LOGMSGIDI("[QA_LOG] Pressure Sensor FAIL\r\n", 0);
            break;
        case SENSOR_TYPE_PROXIMITY:
            LOGMSGIDI("[QA_LOG] Proximity Sensor FAIL\r\n", 0);
            break;
        default:
            LOGMSGIDI("[QA_LOG] FAIL\r\n", 0);
            break;
        }
        return 0;
    }
