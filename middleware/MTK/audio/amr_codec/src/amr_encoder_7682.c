...
{
    LOGMSGIDI("amr_encoder_task_main\r\n", 0);
...
void amr_encoder_task_create(void)
{
    if (amr_encoder_task_handle ==  NULL) {
        LOGMSGIDI("create amr encoder task\r\n", 0);
...
amr_encoder_status_t amr_encoder_get_memory_size(uint32_t *memory_size)
{
    ...
    if (amr_encoder_internal_handle == NULL) {
        LOGMSGIDE("amr_encoder_get_memory_size error,  amr_encoder_internal_handle should not be NULL\r\n", 0);
...
static void amr_encoder_hisr_handler(void *data)
{
    ...
    if ((ring_buffer_get_space_byte_count(&internal_handle->encoded_pcm_buff)) >= 32) { //32bytes is the possible maximum size currently
        ...
        if (total_share_buff_read_len < 320) {
            LOGMSGIDW("share_buff(%d) < 320 bytes\r\n",1, total_share_buff_read_len);
...
{
    LOGMSGIDI("[CTRL]amr_encoder_flush \r\n", 0);
    amr_encoder_internal_handle_t *internal_handle = amr_encoder_internal_handle;
...
{
    LOGMSGIDI("[CTRL]amr_encoder_start \r\n", 0);
    if (handle->state != AMR_ENCODER_STATE_READY) {
        LOGMSGIDE("codec state != AMR_ENCODER_STATE_READY\r\n", 0);
        return AMR_ENCODER_STATUS_ERROR;
    }
...
{
    LOGMSGIDI("[CTRL]amr_encoder_open \r\n", 0);
    ...
    if (internal_handle == NULL) {
        LOGMSGIDE("pvPortMalloc internal_handle failed\r\n", 0);
        return NULL;
    }
    ...
    if (internal_handle->encoded_pcm_buff.buffer_base_pointer == NULL) {
        LOGMSGIDE("Malloc encoded_pcm_buff failed\r\n", 0);
    ...
    if (internal_handle->enc_int_buffer == NULL) {
        LOGMSGIDE("Malloc enc_int_buffer failed\r\n", 0);
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
    ...
    if (internal_handle->enc_tmp_buffer == NULL) {
        LOGMSGIDE("Malloc enc_tmp_buffer failed\r\n", 0);
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
    ...
    if (internal_handle->enc_pcm_buffer == NULL) {
        LOGMSGIDE("Malloc enc_pcm_buffer failed\r\n", 0);
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
    ...
    if (internal_handle->enc_bs_buffer == NULL) {
        LOGMSGIDE("Malloc enc_bs_buffer failed\r\n", 0);
        amr_encoder_delete_memory_buffer();
        return NULL;
    }
...
{
    LOGMSGIDI("[CTRL]amr_encoder_close \r\n", 0);
