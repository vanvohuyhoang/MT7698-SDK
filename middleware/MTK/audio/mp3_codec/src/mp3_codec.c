...
#if defined(MTK_AVM_DIRECT)
static uint8_t mp3_codec_decode[32064];
            ...
            if (handle->state == MP3_CODEC_STATE_PLAY) {
                ...
                if (stream_out_pcm_buffer_data_count == 0) {
...
#if defined(MTK_AVM_DIRECT)
                    {
                        ...
                        p_info = hal_audio_query_share_info(internal_handle->message_type);
...
#endif
{
    ...
    size_memory_pool += (internal_handle->decode_pcm_buffer_size
                + internal_handle->working_buff1_size
                + internal_handle->working_buff2_size
                        + internal_handle->stream_out_pcm_buff_size);
#else
    ...
    if(size_memory_pool > sizeof(mp3_codec_decode)){
        ...
        platform_assert("MP3 codec memory size error!!!",__FILE__,__LINE__);
    }
...
void mp3_codec_event_send_from_isr(mp3_codec_queue_event_id_t id, void *parameter)
{
    ...
    if (mp3_handle_ptr_to_queue_handle(parameter) != NULL) {
        ...
        if(pdFALSE == xQueueIsQueueFullFromISR(mp3_handle_ptr_to_queue_handle(parameter))){
            xQueueSendFromISR(mp3_handle_ptr_to_queue_handle(parameter), &event, 0);
...
#ifdef MTK_MP3_TASK_DEDICATE
extern TimerHandle_t VP_AWS_SYNC_xOneShotTimer;
...
static void mp3_codec_decode_open_handler(void *data)
{
    ...
    if(handle->state == MP3_CODEC_STATE_PLAY) {
        if(VP_AWS_SYNC_xOneShotTimer != NULL){
            xTimerStatus = xTimerStop( VP_AWS_SYNC_xOneShotTimer, 0 );
            if(xTimerStatus != pdPASS){
                MP3_LOG_E("[VPC][SYNC]Timer Stop error.\r\n", 0);
            }
        }
    ...
    if(handle->aws_sync_request == true){
        TickType_t xTimeNow;
        xTimeNow = xTaskGetTickCount();
        if(handle->aws_sync_time > (xTimeNow)){
            xTimerChangePeriod( VP_AWS_SYNC_xOneShotTimer,
                                handle->aws_sync_time - xTimeNow,
                                0 );
            xTimerStatus = xTimerStart( VP_AWS_SYNC_xOneShotTimer, 0 );
            //LOG_I(VPC, "[VPC][SYNC]Timer start xTimeNow(%d).\r\n", xTaskGetTickCount());
            if(xTimerStatus != pdPASS){
                MP3_LOG_E("[VPC][SYNC]Timer start error.\r\n", 0);
            }
        } else {
            MP3_LOG_I("[VPC][SYNC]Timer out. (open ack too late.)\r\n", 0);
            mp3_codec_decode_sync_timeout_handler((void *)g_mp3_codec_task_handle);
#if 0
...
static void mp3_codec_decode_stop_handler(void *data)
{
    ...
    if((handle->state == MP3_CODEC_STATE_PLAY) || (handle->state == MP3_CODEC_STATE_ERROR)){
        ...
        if(internal_handle->media_bitstream_end == true){
            ...
            MP3_LOG_I("[VPC]VP stop request wait vp data rang down.", 0);
...
static void mp3_codec_play_avm(mp3_codec_media_handle_t *handle)
{
    ...
    open_param.stream_out_param.afe.stream_out_sampling_rate   = internal_handle->sampling_rate;
    open_param.stream_out_param.afe.sampling_rate   = internal_handle->sampling_rate;
    ...
    hal_audio_dsp_dl_clkmux_control(AUDIO_MESSAGE_TYPE_PROMPT, open_param.stream_out_param.afe.audio_device, sampling_rate_enum_to_value(internal_handle->sampling_rate), true);
