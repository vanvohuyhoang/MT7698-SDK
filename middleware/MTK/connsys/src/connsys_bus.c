...
#define CONNSYS_DMA_MAX_WAIT_TIME_COUNT  (2000000)

// ========================== SDIO Private Routines =============================
static void connsys_bus_log_info(uint32_t reg)
{
#if (PRODUCT_VERSION == 7687 || PRODUCT_VERSION == 7697)
    uint32_t cr1 = REG32(GDMA_CH4_COUNT);
    uint32_t cr2 = REG32(GDMA_CH4_CON);
    uint32_t cr3 = REG32(GDMA_CH4_RLCT);
    ...
    uint32_t cr1 = REG32(GDMA_CH6_COUNT);
    uint32_t cr2 = REG32(GDMA_CH6_CON);
    uint32_t cr3 = REG32(GDMA_CH6_RLCT);
    uint32_t cr4 = REG32(0xA0020650);
    uint32_t cr5 = REG32(0xC0C00000);
    ...
    printf("reg:%x, %x, %x, %x, %x, %x\r\n",
           ...
           (unsigned int)cr5);
#else
    // Reserved for the future chip
#endif
}


/* Check if N9 is ready for connsys read / write. When N9 is sleep, its register is power off and 0 will be returned.
  * Therefore, use this feature to check if N9 is ready for connsys read / write or not.
  */
static bool connsys_bus_is_n9_ready(void)
{
#if (PRODUCT_VERSION == 7687 || PRODUCT_VERSION == 7697)
#define CONNSYS_MCU_CONFIG_BASE         (0x80000000)
#define CONNSYS_MCU_CONFIG_HW_VER       CONNSYS_MCU_CONFIG_BASE
#elif (PRODUCT_VERSION == 7698 || PRODUCT_VERSION == 7686 || PRODUCT_VERSION == 7682 || PRODUCT_VERSION == 5932)
#define CONNSYS_MCU_CONFIG_BASE         (0xC0070000)
#define CONNSYS_MCU_CONFIG_HW_VER       CONNSYS_MCU_CONFIG_BASE
#else
// Reserved for the future chip
#endif
...
#elif (PRODUCT_VERSION == 7698 || PRODUCT_VERSION == 7686 || PRODUCT_VERSION == 7682 || PRODUCT_VERSION == 5932)
    uint32_t hw_ver = REG32(CONNSYS_MCU_CONFIG_HW_VER);
...
int32_t connsys_bus_read_port_pio(connsys_gen3_cmd53_info *info, void *dst, int32_t count)
{
    int32_t i;
    ...
    if (!info || 0 >= count)
    {
        return CONNSYS_STATUS_INVALID_PARAMETER;
    }

    local_irq_save(flags);
    if (!connsys_bus_is_n9_ready())
    {
        local_irq_restore(flags);
        return CONNSYS_STATUS_WRONG_STATE;
    }
...
int32_t connsys_bus_read_port_dma(connsys_gen3_cmd53_info *info, void *dst, int32_t count)
{
    uint32_t flags = 0;
    uint32_t reg = 0;
    uint32_t count_start = 0, count_stop = 0, count_duration = 0;

    /* Check if (0 == (count >> 2)) to avoid setting 0 to the DMA count CR. */
    if (!info || 0 >= count || (0 == (count >> 2)))
{
        printf("count:%d\r\n", (int)count);
        return CONNSYS_STATUS_INVALID_PARAMETER;
    }
...
//	REG32(GDMA_CH4_CON) |= (0x1 << 15); 	   /* Enable DMA finish interrupt */
    ...
    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &count_start);
    do {
        reg = REG32(GDMA_CH4_RLCT);
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &count_stop);
        hal_gpt_get_duration_count(count_start, count_stop, &count_duration);		
    } while (reg != 0x00 && CONNSYS_DMA_MAX_WAIT_TIME_COUNT >= count_duration);
#elif (PRODUCT_VERSION == 7698 || PRODUCT_VERSION == 7686 || PRODUCT_VERSION == 7682 || PRODUCT_VERSION == 5932)
    ...
    local_irq_restore(flags);
    if (reg != 0x00) {
        connsys_bus_log_info(reg);
        return CONNSYS_STATUS_DMA_FAILURE;
    }
                              ...
                              int32_t size)
{
    ...
    int32_t count = 0, ret = CONNSYS_STATUS_FAIL;
    ...
    if (func->use_dma) {
        ...
        */
        ...
        ret = connsys_bus_read_port_dma(&info, dst, count);
        /*
        	if (drop_count >  0)
        	{
        ...
        */
        ...
        ret = connsys_bus_read_port_pio(&info, dst, count);
    }
...
#endif
extern int inband_queue_inband_handler(void *pkt_ptr, unsigned char *payload, unsigned int len);
...
static int32_t connsys_bus_write_port_pio(connsys_gen3_cmd53_info *info, void *src, int32_t count)
{
    int32_t i;
    uint32_t flags = 0;
    uint32_t value;
...
static int32_t connsys_bus_write_port_dma(connsys_gen3_cmd53_info *info, void *src, int32_t count)
{
    uint32_t reg = 0;
    uint32_t flags = 0;
    uint32_t count_start = 0, count_stop = 0, count_duration = 0;
...
int32_t connsys_bus_write_port_transfer(struct connsys_func *func, connsys_gen3_cmd53_info *info, void *src, int32_t size)
{
    if (func->use_dma) {
        return connsys_bus_write_port_dma(info, src, size);
    } else {
        return connsys_bus_write_port_pio(info, src, size);
    }
}
...
//MT7687_BORA00007905
#if (PRODUCT_VERSION == 7698 || PRODUCT_VERSION == 7686 || PRODUCT_VERSION == 7682 || PRODUCT_VERSION == 5932)
   uint32_t use_dma = func ? func->use_dma : 0;
#endif
...
#endif
	{
        ret = connsys_bus_write_port_dma(&info, src, count);
    } else {
        ret = connsys_bus_write_port_pio(&info, src, count);
    }
 ...
 if(Cid != 0xff)
 {
  ...
  else
  {
    ...
    for (buf_idx = 0; buf_idx < tx_info->buf_num; buf_idx++) {
        ...
        if (func->use_dma) {
            unsigned int reg = 0;
            unsigned int len_moved_by_dma = 0;

            /* remain_send_len <= 3 makes sure remain_send_len or (remain_send_len >> 2) will not be 0 when using DMA block mode. */
            if ((!IS_ALIGN_4((uint32_t)src)) || (remain_send_len <= 3)){
...
#if (CONNYS_WSAP00041276 == 1)
                        /* Minimal sending unit is 4 bytes. If there's 1~3 bytes left in the current loop
                         * while sending tx_info->buff[i].buf_ptr, it will be sent in the next loop while
                         * sending tx_info->buff[i + 1].buf_ptr.
                         */
