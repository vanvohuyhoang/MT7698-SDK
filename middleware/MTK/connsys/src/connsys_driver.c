...
void send_msg_to_wifi_task(void);
...
void wifi_eint_handler(void *parameter)
{
    ...
    /* No need to send msg to wifi task. EINT is used to wake up CM4. And when this handler is called, CM4 has been woken up already. */
    //send_msg_to_wifi_task();
}
#endif
void register_wifi_intr_handler(void)
{
...
void connsys_register_wifi_irq(void)
{
...
/* Get the available HIF PSE page count at the moment. There may be data on the way. Therefore,
  * theoretically CM4 should not send the data according to the value of this register.
  */
uint32_t connsys_tx_flow_control_get_available_pse_size(void)
{
    uint32_t pse_hif_available = 0;
    pse_hif_available = (REG32(PSE_RSV_CNT) & (0xFFF));
    return pse_hif_available;
}
...
int32_t connsys_tx_flow_control_check_and_update_tx(int32_t port, uint32_t pkt_len)
{
    ...
    local_irq_save(flags);
    pse_hif_available = connsys_tx_flow_control_get_available_pse_size();
    ...
    if (WIFI_HIF_TX_PORT0_IDX != port &&
        (send_page > ctrl->available_page_cnt - 10 ||
         send_page > (int32_t)pse_hif_available - 10))
    {
        /* There's about 10 PSE pages in the data path. Therefore, reserve 10 PSE pages to
         * avoid sending data to PSE when there's no PSE page available.
                 */
...
extern void spm_control_mtcmos_internal(spm_mtcmos_type_t mtcmos, spm_mtcmos_ctrl_t ctrl);
#endif
void radio_on_off(UCHAR Radio)
{
    if (Radio == 1)
    {
        ...
        hal_gpt_delay_ms(1);  // avoid system hang when WiFi do opmode switch
#ifdef HAL_SLEEP_MANAGER_ENABLED
...
uint32_t connsys_reload_fw(void)
{
    HAL_REG_32(AON_TOP_AON_RSV) &= ~N9_HIF_RDY; //Need to reset this bit when doing radio off or fw reload.
    hal_gpt_delay_ms(1);  // avoid system hang when WiFi do opmode switch
#ifdef HAL_SLEEP_MANAGER_ENABLED
