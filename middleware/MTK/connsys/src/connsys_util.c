...
void connsys_util_fw_download_done()
{
    ...
    int32_t ret;
    uint32_t gpt_start_count= 0, gpt_stop_count = 0;
...
{
    uint8_t err = 1, wait_tx_done, wait_rx_done, get_semaphore, status = 0;
...
void connsys_patch_direct_download(void)
{
...
void wifi_print_mgmt_queue(void);
...
void ConnsysFlushReorderingBuffer(void);
#endif
...
int32_t connsys_util_intr_enhance_mode_receive_data(void)
{
    ...
    ssize_t  err;
    uint32_t retry;
    //uint32_t rx_irq_flags = 0; /* It will assert if call os API with the interrupt disabled in SDK. */
        ...
        if (cr_whisr & (RX0_DONE_INT | RX1_DONE_INT)) {
            ...
            while (1) {
                ...
                if (err || !rx_len) {
                    ...
                    // Enable Interrupt
                    //local_irq_restore(rx_irq_flags);
                    connsys_enalbe_whier_tx_int();
                    connsys_enalbe_whier_rx_int();
#if defined (MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE_BY_SHARE_MEM)
#if (PRODUCT_VERSION == 7687 || PRODUCT_VERSION == 7697)
...
int32_t connsys_util_low_level_output(uint8_t *buf, ssize_t len, int32_t inf_num)
{
...

    {
        ...
        flow_control_status =
            connsys_tx_flow_control_check_and_update_tx(WIFI_HIF_TX_PORT1_IDX, real_tx_len);
...
#if 0
//for time profileing debug
#define TIME_RECORD_ARRAY_SIZE  (10)
...
TIME_STAMP time_profile[TIME_RECORD_ARRAY_SIZE] = {0};

void connsys_record_time_stamp(uint8_t record_seq)
{
    TIME_STAMP * timestamp = &time_profile[record_seq];

    uint32_t current_time = 0;
    uint32_t temp_chain_diff = 0;
    uint32_t temp_yoy_diff = 0;

    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &current_time);
    timestamp->local_counter ++;

    //1. cal and record the duratian average with the before one, and the duration average with before cycle
    if(record_seq == 0) {
        temp_chain_diff = 0;
        timestamp->avg_chain = 0;
    } else {
        temp_chain_diff = (current_time >= (timestamp-1)->current_time) ? (current_time - (timestamp-1)->current_time) : ((0xffffffff - ((timestamp-1)->current_time - current_time)) + 1);
    }

    temp_yoy_diff   = (current_time >= timestamp->current_time) ? (current_time - timestamp->current_time) : ((0xffffffff - (timestamp->current_time - current_time)) + 1);

    if(timestamp->local_counter > 2) {
        //avg calc
        timestamp->avg_chain = (timestamp->avg_chain + temp_chain_diff) >> 2;
        timestamp->avg_yoy   = (timestamp->avg_yoy + temp_yoy_diff) >> 2;
    } else if(timestamp->local_counter > 1) {
        //avg first record
        timestamp->avg_chain = temp_chain_diff;
        timestamp->avg_yoy   = temp_yoy_diff;
    } else {
        //one number
        timestamp->avg_chain = 0;
        timestamp->avg_yoy = 0;
...
{
    uint8_t i = 0;
    while(time_profile[i].avg_yoy != 0 && i < TIME_RECORD_ARRAY_SIZE) {
        printf("seq=%d, count=%d, avg_chain=%d, avg_yoy=%d\n", i, time_profile[i].local_counter, time_profile[i].avg_chain, time_profile[i].avg_yoy);
        i++;
    }
}
#endif
