...
/* Indicate if the NVDM of the just updated FW is incompatible with the one of the previous FW. 
  * If it is, such as NVDM item structure change, NVDM module should call a customer implementated callback
  * during the first boot-up of the just updated FW. And the customers should store the old NVDM item values
  * into the new NVDM items.
  */
#define FOTA_MULTI_INFO_NVDM_INCOMPATIBLE_FLAG_OFFSET  0x280

/* Add new info type offset before this line and update the offset below. */
#define FOTA_MULTI_INFO_NULL_ADDR_OFFSET  0x300
...
FOTA_ERRCODE fota_multi_info_sector_clean(uint16_t not_wb_info_types, bool *erase_done)
{
    ...
    /* nvdm_incompatible_flag */
    if (!(FOTA_MULTI_INFO_TYPE_NVDM_INCOMPATIBLE_FLAG & not_wb_info_types))
    {
        nvdm_incompatible_flag_set = fota_nvdm_incompatible_flag_is_set();
    ...
    /* Write back nvdm_incompatible_flag */
