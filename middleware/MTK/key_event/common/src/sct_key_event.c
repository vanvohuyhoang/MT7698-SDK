...
bool sct_key_event_init(void)
{
    if(sct_key_event_state.has_initilized == true) {
        LOG_MSGID_I(common, "[key][sct]sct_key_event_init has been initilized\r\n", 0); 
        return false;
    }
...
bool sct_key_set_event_time(sct_key_event_time_t* event_time)
{
    ...
    if (event_time == NULL) {
        LOG_MSGID_I(common, "[key][sct]event_time is null\r\n", 0);    
        return false;       
    }
...
bool sct_key_set_driven_signal_time(uint32_t signal_time)
{
    ...
    if (signal_time <= SCT_KEY_DEFAULT_DEBOUNCE_TIME) {
        LOG_MSGID_I(common, "[key][sct]setting signal time %d < %d \r\n",2,signal_time, SCT_KEY_DEFAULT_DEBOUNCE_TIME);   
        return false;
    }
