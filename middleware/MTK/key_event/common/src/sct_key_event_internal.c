...
static void sct_key_allocate_timer(uint8_t index)
{
    ...
    if (ret_state != HAL_GPT_STATUS_OK) {
        LOG_MSGID_I(common, "[key][sct]get timer handle error,ret = %d, handle = 0x%x\r\n",2, \
...
static void sct_key_start_timer(uint8_t index, uint32_t time)
{
    ...
    if (ret_state != HAL_GPT_STATUS_OK) {
        LOG_MSGID_I(common, "[key][sct]start timer handle error,ret = 0x%x, sate=%d\r\n",2, ret_state, sct_key_event_state.sct_state[index]);
    }   
...
static void sct_key_stop_timer(uint8_t index)
{
    ...
    if (ret_state != HAL_GPT_STATUS_OK) {
        LOG_MSGID_I(common, "[key][sct]stop timer handle error,ret = %d, handle = 0x%x\r\n",2, \
                     (int)ret_state, \
...
static void sct_key_free_timer(uint8_t index)
{
    ...
    if (ret_state != HAL_GPT_STATUS_OK) {
        LOG_MSGID_I(common, "[key][sct]free timer handle error,ret = %d, handle = 0x%x\r\n",2, \
                     (int)ret_state, \
                     (int)sct_key_event_state.timer_handle[index]);
    }
}
...
static void sct_key_process_key(sct_key_mapping_event_t *key_event, bool is_mapping_tale)
{
    ...
    if (key_event->state == SCT_KEY_DRIVEN_PRESS) {
        ...
        LOG_MSGID_I(common, "[key][sct]sct_state_1 = %d \r\n",1, sct_key_event_state.sct_state[index]);
    ...
    LOG_MSGID_I(common, "[key][sct]press time %d \r\n",1, sct_key_event_state.press_time);
    ...
    while (1) {
        if (xQueueReceive(sct_key_event_queue_handle, &msgs, portMAX_DELAY)) {
            switch (msgs.msg_id) {
                ...
                case SCT_CALL_CALLBACK: {
                    ...
                    if (sct_key_event_state.sct_state[index] == SCT_KEY_SHORT_PRESS) {
                        ...
                        LOG_MSGID_I(common, "[key][sct]wrong event in timer callback]\r\n", 0);
                        assert(0);
                    }
                    sct_key_event_state.sct_state[index] = SCT_KEY_RELEASE;
            ...
            if (stack_cur > stack_max) {
                stack_max = stack_cur;
                LOG_MSGID_I(common, "key sct task max-usage:%d \r\n",1, stack_max);
            }
        }
    }
