...
TaskHandle_t  smtcn_switch_ch_handle        = NULL;
QueueHandle_t smtcn_switch_ch_queue         = NULL;
...
#endif
void switch_next_channel_task(void *pvParameters)
{
    uint8_t channel;
    bool bExit = false;
    if(smtcn_switch_ch_queue == NULL) {
        smtcn_switch_ch_queue = wifi_os_queue_create(4, sizeof(uint8_t));
    }
    if (smtcn_switch_ch_queue == NULL) {
        printf("smtcn_switch_ch_queue create fail.\r\n");
        vTaskDelete(NULL);
        return;
    }
    while(bExit == false){
        if (wifi_os_queue_receive(smtcn_switch_ch_queue, &channel, NULL) == 0) {
            if(channel == 0xff) {
                bExit = true;
                if(smtcn_switch_ch_queue != NULL) {
                    wifi_os_queue_delete(smtcn_switch_ch_queue);
                    smtcn_switch_ch_queue = NULL;                    
                    smtcn_switch_ch_handle = NULL;
                }
            }else {
...
static void switch_next_channel(TimerHandle_t tmr)
{
    ...
    if(smtcn_switch_ch_queue != NULL) {
        if(xQueueSend(smtcn_switch_ch_queue, &smt_conn_st.cur_chanl, 0) != pdPASS) {
            ret = -1;
        }
    } else {
        printf("please intial switch channel task first.\r\n");
        return;
    }
    if(ret != 0) {
...
void wifi_smart_connection_deinit(void)
{
    ...
    if(smtcn_switch_ch_handle != NULL) {
        uint8_t deinit = 0xff;
        wifi_os_queue_send(smtcn_switch_ch_queue, &deinit, 0);
    }
...
wifi_smart_connection_status_t wifi_smart_connection_init (const uint8_t *key, const uint8_t key_length, wifi_smart_connection_callback_t  callback)
{
    ...
    if(smtcn_switch_ch_handle == NULL) {
        if ((xTaskCreate(switch_next_channel_task,
                        "start smnt switch CH task",
                        512,
                        NULL,
                        TASK_PRIORITY_NORMAL,
                        &smtcn_switch_ch_handle) )!= pdPASS) {
            printf("start smnt switch CH task fail.\r\n");
            return WIFI_SMART_CONNECTION_ERROR;
        }
    }
...
wifi_smart_connection_status_t wifi_smart_connection_start (uint16_t timeout_seconds)
{
    ...
    do
    {
        if (0 > wifi_config_get_opmode(&origin_opmode)) {
