...
T_WFC_CONFIG *wfcm_get_config(uint16_t opcode)
{
    ...
    LOG_E(wfcm,"Get g_wfcm_stub_opcode fail.");
    return NULL;
}
...
void wfcm_hrx_handler(uint8_t *pbuf, uint32_t bufSize)
{
    ...
    if (!pbuf)
    {
        LOG_E(wfcm,"[HRX]: Null Packet Received!");
        return;
    }
    ...
    LOG_I(wfcm,"HRX data, size = %d.",bufSize);
...
uint16_t wfcm_send_cmd_to_slave(T_WFC_OPCODE *pwfc, uint8_t qbuf_release)
{
    ...
    // Send Host Command
    if(wfcm_spi_send_data(&(pwfc->qbuf), qbuf_release) == false){
        LOG_E(wfcm,"WFCM send CMD fail.");
    }

    // Receive Command Response
    if(wfcm_spi_receive_cmdrsp(rspbuf, &rspSize) == false){
        LOG_E(wfcm,"WFCM rcv CMD rsp fail.");
    }
#else
    // Send Host Command
    if(wfcm_sdio_send_data(&(pwfc->qbuf), qbuf_release) == false){
        LOG_E(wfcm,"WFCM send CMD fail.");
    }

    // Receive Command Response
    if(wfcm_sdio_receive_cmdrsp(rspbuf, &rspSize) == false){
        LOG_E(wfcm,"WFCM rcv CMD rsp fail.");
    }
#endif
...
{
#ifdef MTK_WIFI_STUB_CONF_SPI_ENABLE
    if(wfcm_spi_send_data(qbuf, true) == false){
        LOG_E(wfcm,"WFCM send data fail.");
        return WFC_RSP_PARAM_ERR;
    }
#else
    if(wfcm_sdio_send_data(qbuf, true) == false){
        LOG_E(wfcm,"WFCM send data fail.");
        return WFC_RSP_PARAM_ERR;
    }
#endif
...
uint16_t wfcm_to_slave(uint16_t op, uint8_t *ptr, uint32_t ptrSize, uint8_t *rsp, uint32_t *rspSize, uint8_t fromISR)
{
    ...
    {
    	LOG_E(wfcm,"[WFCM]: To Slave Fail - %u, %u.", (unsigned int)wfcm_queue, (unsigned int)g_wfcm_sem_rsp);
    ...
    if (fromISR)
    {
        ...
        {
            LOG_E(wfcm,"Send wfcm_queue fail.");
            return WFC_RSP_QUEUE_ERR;
        }
...
void wfcm_tx_handler(T_WFC_OPCODE *wfcm_qbuf)
{
    if((wfcm_qbuf->pwfg->type == WFC_CT_SYSTEM) && (wfcm_qbuf->opcode == WFC_CMD_DTW_DATA)) {
        LOG_I(wfcm,"Send data to slave,size = %d.",wfcm_qbuf->txsize);
        wfcm_send_data_to_slave(&(wfcm_qbuf->qbuf));
    }else if(wfcm_qbuf->pwfg->type != WFC_CT_SYSTEM){
        LOG_I(wfcm,"Send CMD to slave.");
        *(wfcm_qbuf->rspcode) = wfcm_send_cmd_to_slave(wfcm_qbuf, true);
        ...
        LOG_E(wfcm,"[WFCM]TX unkown type.%d,%d.",wfcm_qbuf->pwfg->type,wfcm_qbuf->opcode);
    }
...
void wfcm_queue_handler(void)
{
    ...
    if (wfcm_queue==NULL)
    {
        LOG_E(wfcm,"Create wfcm_queue Fail!");
        vTaskDelete(NULL);
        return;
    }
    ...
    while (!bExit)
    {
        ...
        {
            LOG_I(wfcm,"RCV wfcm_queue, opcode:%d.",wfcq.opcode);
            pwfg = wfcq.pwfg;
...
//#endif                    
                    case WFC_CMD_DTH_READY:
                    {
...
#endif
                        LOG_I(wfcm,"RCV data size = %d.",(int)hrxSize);
                        //Allocate Data Buffer (pbuf/mem)
                        p = wfcm_hrx_data_alloc(hrxSize);
                        if (!p)
                        {
                            LOG_E(wfcm,"Data Buffer Allocate Fail!,size = %d.", (int)hrxSize);
                            //break;
                        }
                        ...
                        if(*op == (WFC_RSP_DTH_DATA|WFC_RSP_OK)) {
                            ...
                            if (!hrxSize)
                            {
                                LOG_E(wfcm,"[Error]: Get HRX Size Fail.");
                                break;
                            }
                            ...
                            if (!p)
                            {
                                LOG_E(wfcm,"[WFCM]: Data Buffer Allocate Fail!,size = %d.", (int)hrxSize);
                                //break;
                            }
                            while(!p) {    //Pengfei: workaround for the case that allocate buffer when do iperf
                                p = wfcm_hrx_data_alloc(hrxSize);
                            ...
                            LOG_E(wfcm,"Unkown response code,%d.\r\n",*op);
                            break;
                        }
#endif
                    }
                    case WFC_CMD_TASK_DEINIT:
                    {
                        ...
                        LOG_I(wfcm,"Delete wfcm_stub_task task.\r\n");                       
                    }
                    default:
                    	break;
                }
...
static void wfcm_stub_task(void)
{
    ...
    LOG_I(wfcm,"WFCM interface init.");
    ret = wfcm_if_init();
    if (ret)
    {
        LOG_E(wfcm,"WFCM Interface Init Fail!");
        vTaskDelete(NULL);
        return;
    }
...
uint16_t wfcm_evt_send(uint16_t evt_id, uint8_t *ptr, uint32_t ptrSize, uint8_t fromISR)
{
    ...
    if (!wfcm_evt_queue)
    {
        LOG_E(wfcm,"[WFCM]: Event Send Fail - %u.", (unsigned int)wfcm_evt_queue);
        return WFC_RSP_QUEUE_ERR;
    }
    ...
    LOG_I(wfcm,"Send evt queue, id = %d, size = %d.",evt.eid,evt.size);
    // Send to WFC Queue
    if (fromISR)
    {
        if ( wifi_os_queue_send_from_isr(wfcm_evt_queue, &evt) != 0 ) {
            LOG_E(wfcm,"Send evt queue from ISR fail.");
            return WFC_RSP_QUEUE_ERR;
        }
    }
    else if ( wifi_os_queue_send(wfcm_evt_queue, &evt, 0) != 0 )
    {
        LOG_E(wfcm,"Send evt queue fail.");
        return WFC_RSP_QUEUE_ERR;
    }
...
static void wfcm_evt_task(void)
{
    ...
    {
        LOG_E(wfcm,"wfcm_evt_queue create fail.");
        vTaskDelete(NULL);
        return;
    }
    ...
    while (!bExit)
    {
        if (wifi_os_queue_receive(wfcm_evt_queue, &evt, NULL) == 0)
        {
            ...
            switch (evt.eid)
            {
                ...
                {
                    LOG_I(wfcm,"WFC_EVT_INIT_DONE event.");
...
void wfcm_stub_deinit(void)
{
    ...
    {
        LOG_E(wfcm,"Send delete command fail.");
    }
    
    if ( wifi_os_queue_send(wfcm_evt_queue, &delete_evt, 0) != 0 )
    {
        LOG_E(wfcm,"Send delete evt fail.");
    }
