...
int wifi_scan_list_handler(unsigned int inf, unsigned char *body, int len)
{
    ...
    if (//(g_scan_array[empty_slot].beacon.hidden_ssid) &&
            ...
            (os_memcmp(&g_scan_array[empty_slot].beacon.Bssid[0], &pBeacon->Bssid[0], WIFI_MAC_ADDRESS_LENGTH) == 0) ) {
        ...
        pBeacon->LengthVIE = g_scan_array[empty_slot].beacon.LengthVIE;/*For 5G hidden AP need too much time to connect success*/
    }
