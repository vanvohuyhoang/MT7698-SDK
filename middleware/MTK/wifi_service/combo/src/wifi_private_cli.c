...
extern int32_t port_sanity_check(char *port_str);
/**
* @brief  termiante wps connect process
* @parameter
* @return =0 means success, >0 means fail
*/
uint8_t wifi_config_wps_terminate_connect_ex(uint8_t len, char *param[])
{
    ...
    if( len < 1)
    {
        printf("Usage:  wifi wps terminate connect <port>\n");
        return 1;
    }

    if ((port = port_sanity_check(param[0])) < 0)
        return port;

    if (port != WIFI_PORT_STA)
    {
        printf("current only support STA mode\n");
        return 1;
    }

    return wifi_wps_terminate_connect(port);
}
...
static uint8_t wifi_config_set_single_sku_table_ex(uint8_t len, char *param[])
{
    ...
    int32_t ret = 0;
    uint8_t band = 0;
    uint32_t buff_len = PROFILE_BUF_SKU_LEN;
    uint32_t nvdm_buff_len = buff_len;
    ...
    if( len < 1 ){
        printf("Usage: wifi config set single_sku <band, 0:2G, 1: 5G> \n");
        ret = -1;
        goto cli_end;
    }
    ...
    if ( band == WIFI_BAND_2_4_G ) {
        ...
        os_memset(nvdm_buff,0x00,buff_len);

        nvdm_buff_len = buff_len;
...
uint8_t ch_table_5g[42] = {36,38,40,44,46,48,52,54,56,60,62,64,100,102,104,108,110,112,116,118,120,124,126,128,132,134,136,140,142,144,149,151,153,157,159,161,165,169,173};
static uint8_t wifi_config_get_single_sku_table_ex(uint8_t len, char *param[])
{
    ...
    if( len < 1 ){
        printf("Usage: wifi config get single_sku <band, 0:2G, 1: 5G> \n");
        ret = -1;
        goto cli_end;
    }
    band = atoi(param[0]);
...
uint8_t wifi_config_set_mgmt_ac3_on_ex(uint8_t len, char *param[])
{
	...
	if( len < 1 ){
		printf("Usage: wifi config set mgmt_ac3 <onoff> \n");
		return 1;
	}

	flag = atoi(param[0]);
	ret = wifi_config_set_mgmt_ac3_enable(flag);
	if (ret < 0) {
		status = 1;
	}
	printf("wifi_config_set_mgmt_ac3_enable(onoff=%d), ret:%s, Code=%d\n", flag, WIFI_CLI_RETURN_STRING(ret), (int)ret);
	return status;
}
...
uint8_t wifi_config_get_mgmt_ac3_on_ex(uint8_t len, char *param[])
{
	...
	ret = wifi_config_get_mgmt_ac3_enable(&flag);
	if (ret < 0) {
		status = 1;
	}
	printf("wifi_config_get_mgmt_ac3_enable(onoff=%d), ret:%s, Code=%d\n", (int)flag, WIFI_CLI_RETURN_STRING(ret), (int)ret);
	return status;
}
...
uint8_t wifi_config_set_tmac_trcr_on_ex(uint8_t len, char *param[])
{
	...
	if( len < 1 ){
		printf("Usage: wifi config set tmac_trcr <onoff> \n");
		return 1;
	}

	flag = atoi(param[0]);
	ret = wifi_config_set_tmac_trcr_enable(flag);
	if (ret < 0) {
		status = 1;
	}
	printf("wifi_config_set_tmac_trcr_enable(onoff=%d), ret:%s, Code=%d\n", flag, WIFI_CLI_RETURN_STRING(ret), (int)ret);
	return status;
}
...
uint8_t wifi_config_get_tmac_trcr_on_ex(uint8_t len, char *param[])
{
	...
	ret = wifi_config_get_tmac_trcr_enable(&flag);
	if (ret < 0) {
		status = 1;
	}
	printf("wifi_config_get_tmac_trcr_enable(onoff=%d), ret:%s, Code=%d\n", (int)flag, WIFI_CLI_RETURN_STRING(ret), (int)ret);
	return status;
}
...
uint8_t wifi_config_set_efem_ex(uint8_t len, char *param[])
{
	...
	if( len < 1 ){
		printf("Usage: wifi config set eFEM SKY85329-11/SKY85331-11.\r\n");
		return 1;
	}
    
    if (!os_strcmp(param[0], "SKY85329-11")){        
#if (PRODUCT_VERSION == 7686 || PRODUCT_VERSION == 7698)
        hal_pinmux_set_function(HAL_GPIO_1,HAL_GPIO_1_WIFIRF_DBGOUT1);
        hal_pinmux_set_function(HAL_GPIO_4,HAL_GPIO_4_WIFI_ANT_SEL3);
        hal_pinmux_set_function(HAL_GPIO_5,HAL_GPIO_5_WIFI_ANT_SEL0);
        hal_pinmux_set_function(HAL_GPIO_10,HAL_GPIO_10_WIFI_ANT_SEL4);

        wifi_config_set_ant_select_mode(WIFI_ANT_SELECT_INDEX_0,ANT_SELECT_MODE_PHY_RF_SW_RX);
        wifi_config_set_ant_select_mode(WIFI_ANT_SELECT_INDEX_3,ANT_SELECT_MODE_PHY_RF_SW_TX);
        wifi_config_set_ant_select_mode(WIFI_ANT_SELECT_INDEX_4,ANT_SELECT_MODE_PHY_RF_EXT_LNA); 
#else
        printf("eFEM only support in mt7686/aw7698 project.\r\n");
#endif        
    }else if (!os_strcmp(param[0], "SKY85331-11")){
#if (PRODUCT_VERSION == 7686 || PRODUCT_VERSION == 7698)
        hal_pinmux_set_function(HAL_GPIO_0,HAL_GPIO_0_WIFI_ANT_SEL0);
        hal_pinmux_set_function(HAL_GPIO_1,HAL_GPIO_1_WIFIRF_DBGOUT1);
        hal_pinmux_set_function(HAL_GPIO_2,HAL_GPIO_2_WIFI_ANT_SEL4);
        hal_pinmux_set_function(HAL_GPIO_4,HAL_GPIO_4_WIFI_ANT_SEL3);
        
        wifi_config_set_ant_select_mode(WIFI_ANT_SELECT_INDEX_0,ANT_SELECT_MODE_PHY_RF_SW_RX);
        wifi_config_set_ant_select_mode(WIFI_ANT_SELECT_INDEX_3,ANT_SELECT_MODE_PHY_RF_SW_TX);
        wifi_config_set_ant_select_mode(WIFI_ANT_SELECT_INDEX_4,ANT_SELECT_MODE_PHY_RF_EXT_LNA_INVERSE);   
#else
        printf("eFEM only support in mt7686/aw7698 project.\r\n");
#endif        
    }else {
	...
	printf("wifi_config_set_efem_ex(%s), ret:%s, Code=%d\n", param[0], WIFI_CLI_RETURN_STRING(ret), (int)ret);    
    return status;
}
...
cmd_t   wifi_config_set_private_cli[] = {
    ...
    { "wps_termin_connect", "terminate wps connecting", wifi_config_wps_terminate_connect_ex },
#endif
...
#endif
    { "mgmt_ac3",		"mgmt_ac3 enable/disable", 	    wifi_config_set_mgmt_ac3_on_ex },
    { "tmac_trcr",		"tmac_trcr enable/disable", 	wifi_config_set_tmac_trcr_on_ex },
    { "eFEM",		    "eFEM config",          	wifi_config_set_efem_ex },    
#endif
    { NULL }
};
...
#endif
    { "mgmt_ac3",		"get mgmt_ac3 enable/disable",	wifi_config_get_mgmt_ac3_on_ex },
    { "tmac_trcr",		"get tmac_trcr enable/disable", wifi_config_get_tmac_trcr_on_ex },
#endif
    { NULL }
};
