...
{
    return ( port == WIFI_PORT_STA || port == WIFI_PORT_AP || port == WIFI_PORT_APCLI);
}
...
/**
 * @brief Judge whether the channel is valid, simple check channel range supported by 2.4G/5G
 * @return 1 is vaild, 0 is invaild
 */
bool wifi_is_channel_valid(uint8_t channel)
{
    bool    valid = 0;
    int32_t support_5g = wifi_5g_support();

    if ( support_5g >= 0 ) {
        if( (channel >= 36) && (channel <=173) ){   /*Only 5G*/
            valid = 1;
        }else if( (channel >= 1) && (channel <=14) ){ /*5G+2.4G*/
            valid = 1;
        }
    }else{
        if( (channel >= 1) && (channel <=14) ){      /*2.4G*/
            valid = 1;
        }
    }
    return valid;
...
int32_t wifi_profile_set_ssid(uint8_t port, uint8_t *ssid , uint8_t ssid_length)
{
    ...
    /***add by Pengfei, optimize PMK calculate at boot up***/
   {
        ...
        if (NVDM_STATUS_OK != nvdm_write_data_item(WIFI_PROFILE_BUFFER_STA, "PMK_INFO", NVDM_DATA_ITEM_TYPE_STRING,
			...
			if (NVDM_STATUS_OK != nvdm_write_data_item(WIFI_PROFILE_BUFFER_AP, "PMK_INFO", NVDM_DATA_ITEM_TYPE_STRING,
	                            (uint8_t *)pmk_info, WIFI_MAX_LENGTH_OF_SSID+WIFI_LENGTH_PASSPHRASE+WIFI_LENGTH_PMK)) {
	            return WIFI_FAIL;
	        }
		}
