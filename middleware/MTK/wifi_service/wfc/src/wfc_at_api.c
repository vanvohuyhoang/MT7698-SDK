    ...
    {                 
        ...
        case WFC_API_INIT_TX_POWER:     
        {
            if(api_id->set == true) {
                ...
                static uint8_t *tx_power_bin_addr = NULL;
         
                /*4 bytes magic number +  20 bytes wifi_tx_rate_db_config_t*/
                int valid_len = 24; //valid content length in ROM_WIFI_TX_POWER_LENGTH
                if(sizeof(set_tx_power_cmd->tx_power_bin) != valid_len)
                    return;

                if( tx_power_bin_addr == NULL){ //No need free, as pRate_Tbl will be used once opmode switch and power set
                    tx_power_bin_addr = (uint8_t *)wifi_os_malloc(valid_len);
                    if(tx_power_bin_addr == NULL)
                    return;
                }
                os_memcpy(tx_power_bin_addr,set_tx_power_cmd->tx_power_bin,valid_len);               
                wifi_service_set_tx_power_bin((uint32_t *)tx_power_bin_addr);
        ...
        case WFC_API_DEBUG_LEVEL:     
        {
            if(api_id->set == true) {
                wh_set_debug_log_cmd_para_t *set_debug_level_cmd = (wh_set_debug_log_cmd_para_t *)pcmd;
                wh_set_debug_log_cmd_resp_para_t set_debug_level_resp;
                printf("[PF]Set debug level = %d.\r\n",set_debug_level_cmd->debug_level);
                g_wfc_current_dbg_level = set_debug_level_cmd->debug_level;
                set_debug_level_resp.error_code = 0;               
                os_memcpy(txptr, &set_debug_level_resp, sizeof(set_debug_level_resp));
                *txsize = sizeof(set_debug_level_resp);
            } else {
