...
typedef enum {
    ...
    BT_MESH_EVT_PROV_LINK_OPEN,                        /**< Event for redeiving the link open message sent from a provisioner. */
    BT_MESH_EVT_PROV_INVITE,                           /**< Event for receiving the invite message sent from a provisioner. */
    ...
    BT_MESH_EVT_BEARER_ADV_STATUS,                     /**< Event for the mesh bearer ADV status. */
    BT_MESH_EVT_LOW_POWER_STATUS,                      /**< Event for the mesh low power status. */
} bt_mesh_event_id;
 ...
 */
/*!
     @brief Event parameter of mesh event @ref BT_MESH_EVT_PROV_LINK_OPEN.
 */
typedef struct {
    uint32_t link_id;     /**< The link id. */
} bt_mesh_evt_prov_link_open_t;

/*!
     @brief Event parameter of mesh event @ref BT_MESH_EVT_PROV_INVITE.
 */
typedef struct {
    bt_mesh_prov_invite_t invite;     /**< The invite parameter. */
 ...
 */
typedef struct {
    bt_mesh_prov_capabilities_t cap;     /**< The capabilities parameter. */
...
/*!
     @brief Event parameter of mesh event @ref BT_MESH_EVT_BEARER_ADV_STATUS.
 */
typedef struct {
    bt_mesh_bearer_adv_status_t status;          /**< The status bearer ADV. */
} bt_mesh_evt_bearer_adv_status_t;

/*!
     @brief Event parameter of mesh event @ref BT_MESH_EVT_FRIENDSHIP_STATUS.
     @note The address field shall be ignored when the received status is BT_MESH_FRIENDSHIP_REQUEST_FRIEND_TIMEOUT or BT_MESH_FRIENDSHIP_SELECT_FRIEND_TIMEOUT.
 */
typedef struct {
...
#if 0
/*!
     @brief Event parameter of mesh event @ref BT_MESH_EVT_LOW_POWER_STATUS.
 */
typedef struct {
    bt_mesh_low_power_status_t status;         /**< The low power status. */
    bool success;                               /**< Indicate the low power status switching process is successfull or not. */
} bt_mesh_evt_low_power_status_t;
#endif
 ...
 */
typedef struct bt_mesh_evt_t {
    /**
     * @brief Mesh union structure.
     */
    union {
        bt_mesh_evt_prov_link_open_t         prov_link_open;     /**<  Parameter of mesh event @ref BT_MESH_EVT_PROV_LINK_OPEN. */
        bt_mesh_evt_prov_invite_t            prov_invite;        /**<  Parameter of mesh event @ref BT_MESH_EVT_PROV_INVITE. */
        ...
        bt_mesh_evt_bearer_adv_status_t      bearer_adv_status;  /**<  Parameter of mesh event @ref BT_MESH_EVT_BEARER_ADV_STATUS. */
#if 0
        bt_mesh_evt_low_power_status_t       low_power_status;   /**<  Parameter of mesh event @ref BT_MESH_EVT_LOW_POWER_STATUS. */
#endif
    } mesh;
} bt_mesh_evt_t;
...
typedef struct {
    ...
    bt_mesh_friend_init_params_t *friend;                  /**< Initialization parameters of friend. The parameter is available only when Friend feature is supported and enabled. Set this field to NULL when Friend feature is not supported or use default setting. */
 ...
 * @note when mesh is enabled, an event #BT_MESH_EVT_BEARER_ADV_STATUS is received.
 */
 ...
 * @note when mesh is disabled, an event #BT_MESH_EVT_BEARER_ADV_STATUS is received.
 */
