 ...
 *   @{
 *   This section introduces the advertising Bearer layer APIs. It shows on how to use the enums and structures and the API prototypes
 *   to set the advertising parameters and scan parameters for the advertising Bearer layer.
 *
 */
...
typedef struct bt_mesh_bearer_scan_params_t {
...
typedef struct bt_mesh_bearer_adv_params_t {
...
/** @brief Bearer ADV status. */
typedef enum {
    BT_MESH_BEARER_ADV_STATUS_ENABLED = 0,     /**< Bearer ADV is enabled. */
    BT_MESH_BEARER_ADV_STATUS_DISABLED = 1,    /**< Bearer ADV is disabled. */
} bt_mesh_bearer_adv_status_t;
...
/**
 * @brief Sending non-mesh advertising data.
 * @param[in] len is the length of this adv data.
 * @param[in] buf is the adv data.
 * @param[in] retry is the times of resending this adv data.
 * @param[in] params is the advertising parameter of this adv data.
 * @return
 * #BT_MESH_SUCCESS, the adv data is successfully sent. \n
 * #BT_MESH_ERROR_OOM, not enough memory for sending the adv data. \n
 */
bt_mesh_status_t bt_mesh_bearer_adv_send(uint8_t len, uint8_t *buf, uint8_t retry, bt_hci_cmd_le_set_advertising_parameters_t *params);
