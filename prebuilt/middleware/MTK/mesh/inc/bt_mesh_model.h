 ...
 *
 * @section bt_mesh_model_api_usage How to create a mesh device and add models
 *
 ...
 * {
 *     // handle the publish flow in this callback
 *     printf("GenericOnOffStatus(Publish)");
 *
 ...
 *
 *      // create a variable to store your model data
 ...
 *
 *      // adding a SIG Generic OnOff server model with publish handling
 ...
 *      model_params.publish_timeout_cb = _generic_onoff_publish_callback;
 ...
 */
#define BT_MESH_MODEL_SIG_MODEL_ID_SENSOR_SERVER    0x1100    /**< The model ID of the sensor server. */
#define BT_MESH_MODEL_SIG_MODEL_ID_SENSOR_SETUP_SERVER    0x1101    /**< The model ID of the sensor setup server. */
#define BT_MESH_MODEL_SIG_MODEL_ID_SENSOR_CLIENT    0x1102    /**< The model ID of the sensor client. */
...
#define BT_MESH_MODEL_SIG_MODEL_ID_LIGHT_LC_SETUP_SERVER    0x1310    /**< The model ID of the light LC setup server. */
#define BT_MESH_MODEL_SIG_MODEL_ID_LIGHT_LC_CLIENT    0x1311    /**< The model ID of the light LC client. */
...
/*!
    @brief Get model handle of the specified model id.
    @param[in] element_index is the element index of the model.
    @param[in] model_id is the id of the model.
    @return the model handle of the specified model.
*/
uint16_t bt_mesh_model_get_handle(uint16_t element_index, uint32_t model_id);
...
/*!
    @brief Adding subscription for the specified model.
    @param[in] subscription_address is the subscribed address.
    @param[in] model_id is the id of the target model.
    @param[in] unicast_addr is the unicast address of the element.
    @return
    #BT_MESH_SUCCESS, packets successfully sent. \n
    #BT_MESH_ERROR_INVALID_ADDR, the address to subscribe is invalid. \n
*/
bt_mesh_status_t bt_mesh_model_add_subscription(bt_mesh_address_t subscription_address, uint32_t model_id, uint16_t unicast_addr);

/*!
    @brief Adding publication for the specified model.
    @param[in] publish_params are the parameters of publication.
    @param[in] model_id is the id of the target model.
    @param[in] unicast_addr is the unicast address of the element.
    @return
    #BT_MESH_SUCCESS, the publication is successfully added.\n
    #BT_MESH_ERROR_INVALID_ADDR, the address to publish is invalid.\n
    #BT_MESH_ERROR_INVALID_PARAM, the parameters for publication is invalid.\n
*/
bt_mesh_status_t bt_mesh_model_add_publication(bt_mesh_model_publication_t *publish_params, uint32_t model_id, uint16_t unicast_addr);
