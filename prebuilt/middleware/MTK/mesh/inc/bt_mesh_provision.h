...
typedef struct {
    uint8_t attention_duration;  /**< The Attention Timer state, which indicates the attention time in seconds. */
...
/** @brief The provisioning error code used for #bt_mesh_prov_failed_error_t */
typedef enum {
    BT_MESH_PROV_FAILED_ERROR_CODE_PROHIBITED = 0,         /**< Prohibited */
...
/*!
    ...
    @param[in] reason is the reason to stop provisioning process.
    @return
    #BT_MESH_SUCCESS, provisioning successfully stopped. \n
    #BT_MESH_ERROR_FAIL, provisioning stopped fail. \n
*/
bt_mesh_status_t bt_mesh_provision_stop_provisioning(bt_mesh_prov_failed_error_t reason);
...
/*!
    @brief Check if this device is provisioned or not
    @return
    true means this device is provisioned. \n
    false means this device is unprovisioned. \n
*/
bool bt_mesh_provision_is_provisioned(void);
