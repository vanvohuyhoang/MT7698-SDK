...
* - Generic clients are required if your device wishes to send messages to corresponding generic servers to control the functionalities.
* - For example, in order to control another device supporting Generic OnOff server, a Generic OnOff Client would be needed in your device.
* - To create an individual Generic OnOff client, call the API #bt_mesh_model_add_generic_onoff_client() declared in bt_mesh_model_generic_client.h.
* - Like server models, some of generic client models are extended client models, not an individual client.
* - For example, a Light Lightness Client extends Generic Power OnOff Client model as defined in section 6.6.1 of Mesh Model spec.
* - In this case, calling the API #bt_mesh_model_add_generic_power_onoff_client() is not necessary because calling the API #bt_mesh_model_lighting_add_lightness_client() automatically creates Generic Power OnOff Client model.
* - So all APIs for extended models are not required to be called. This rule applies to all mesh models in Airoha SDK.

* - Please refer to sample code below for a sample stucture of property client and its usage:
* -
* -
* - Sample code:
*      @code
   ...
   //examples of commands for sending property messages
   static int cmd_generic_property(void *no, int argc, char** argv)
   {
       //USAGE: generic prop properties <action> <dst_addr> <appidx> <type>
       //USAGE: generic prop property <action> <dst_addr> <appidx> <type> <property_id> [<length> <property_values>]
       ...
       tx_meta.appkey_index = argv[4];
       tx_meta.ttl = bt_mesh_config_get_default_ttl();

       if(!strcmp(argv[1], "properties")) {
           switch(action) {
               case 0: {
                   if (type > 3) {
                       id = argv[6];
                       ret = bt_mesh_model_generic_property_client_get_client_properties(g_model_handle_generic_prop, &tx_meta, id);
                   } else {
                       ret = bt_mesh_model_generic_property_client_get_properties(g_model_handle_generic_prop, &tx_meta, type);
                   }
                   break;
               }
               }
       ...
       else if(!strcmp(argv[1], "property")) {
           id = argv[6];
           switch(action) {
               case 0: {
                   ret = bt_mesh_model_generic_property_client_get_property(g_model_handle_generic_prop, &tx_meta, type, id);
                   break;
               }
               case 1:
               case 2: {
                   bt_mesh_property_setting_params_t params;
                   params.type = type;
                   params.id = id;
                   params.length = argv[7];
                   tmp = argv[8];

                   //copy corresponding value according to type == BT_MESH_DEVICE_PROPERTY_TYPE_USER or
                   // BT_MESH_DEVICE_PROPERTY_TYPE_ADMIN or BT_MESH_DEVICE_PROPERTY_TYPE_MANUFACTURER

                   bt_mesh_model_generic_property_client_set_property(g_model_handle_generic_prop, &tx_meta,
                       &params, (action == 1));
                   break;
               }
           }
   ...
   static void _generic_client_msg_handler(uint16_t model_handle, const bt_mesh_access_message_rx_t *msg, const bt_mesh_generic_client_evt_t *event)
   {
       {
           switch(msg->opcode.opcode)
           ...
           case BT_MESH_GENERIC_CLIENT_EVT_USER_PROPERTY_STATUS:
           case BT_MESH_GENERIC_CLIENT_EVT_ADMIN_PROPERTY_STATUS:
           case BT_MESH_GENERIC_CLIENT_EVT_MANUFACTURER_PROPERTY_STATUS: {
               const bt_mesh_generic_client_evt_property_status_t *evt = &event->data.property_status;
               printf(CUF "PropertyID   : %04x\n", 40, evt->property_id);
               if (msg->length > 2) {
                   printf(CUF "Access       : %02x\n", 40, evt->access);
                   printf(CUF "PropertyValue: \n", 40);
                   bt_mesh_app_util_display_hex_log( evt->property_value, msg->length - 3);
                   free(evt->property_value);
               }
               break;
           }
   ...
   static void mesh_create_device(void)
   {
       ...
       bt_mesh_model_add_generic_battery_client(&g_model_handle_generic_batt, element_index, _generic_client_msg_handler);
       bt_mesh_model_add_generic_location_client(&g_model_handle_generic_loc, element_index, _generic_client_msg_handler);
       bt_mesh_model_add_generic_property_client(&g_model_handle_generic_prop, element_index, _generic_client_msg_handler);
   }
...
/**
 * @defgroup Bluetooth_mesh_generic_model_enum Enum
 * @{
 */
...
/**
 * @defgroup Bluetooth_mesh_generic_model Struct
 * @{
*/
...
typedef struct {
    ...
    uint16_t property_value_length;  /**< The length of the value of the specified property id. */
...
/**
 * @defgroup Bluetooth_mesh_generic_model_typedef Typedef
 * @{
*/
...
/*!
 * @brief Sends a request to get the local or global related fields of the Generic Location state.
 * @param[in] model_handle is the model handle which the message belongs to.
 * @param[in] tx_meta is the metadata of the message.
 * @param[in] local is indicating local or global related fields.
 * @return
 * #BT_MESH_SUCCESS, packets successfully sent. \n
 * #BT_MESH_ERROR_OOM, not enough memory for sending the packet. \n
 * #BT_MESH_ERROR_NULL, tx_params or data is NULL. \n
 ...
 * #BT_MESH_ERROR_INVALID_KEY, invalid key index or no bound network key. \n
*/
bt_mesh_status_t bt_mesh_model_generic_location_client_get(
    uint16_t model_handle, const bt_mesh_access_message_tx_meta_t *tx_meta, uint8_t local);
