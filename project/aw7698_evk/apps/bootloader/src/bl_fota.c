...
void bl_fota_process(void)
{
    ...
    if (fota_is_empty(FOTA_PARITION_TMP) == FOTA_STATUS_IS_FULL) {
        BL_PRINT(LOG_DEBUG, "fota: upgrade\r\n");
    } else {
        BL_PRINT(LOG_DEBUG, "fota: TMP is empty, skip upgrade\r\n");
        return;
    }
    ...
    BL_PRINT(LOG_DEBUG, "fota: start copy\r\n");
    /* start FOTA */
    if (fota_copy(FOTA_PARITION_CM4,
                  FOTA_PARITION_TMP,
                  &buffer[0],
                  UPGRADE_BLOCK_SIZE) == FOTA_STATUS_OK) {
        BL_PRINT(LOG_DEBUG, "fota: upgrade done\r\n");
        return;
    }
    BL_PRINT(LOG_DEBUG, "fota: upgrade failed\r\n");
    return;
}
